function mangafastF() {
    ...
    if(document.querySelector("div.chapter-link-w") !== null){
        rows = document.querySelectorAll("div.chapter-link-w a.chapter-link");
        if(rows[0].href.includes("bilibilicomics")){
            return;
        }
        // calling function
        addNote();
    ...
    function addNote(){
        let chapterDiv = document.querySelector("div.chapter-link-w").parentElement;
        ...
        chapterDiv.insertBefore(note,chapterDiv.querySelector("div.chapter-link-w"));
    }
    ...
    function addDownloadButtons(){
        for (let i=0;i<rows.length;i++) {
            ...
            // store chapter link
            let link = rows[i].href;
            if(link.includes("bilibilicomics")){
                return;
            }

            links[i] = link;
            pdfButton.referrerLink = link;

            pdfButton.title = rows[i].querySelector("div.chapter-w span").textContent.trim();
            ...
            xhttp.onreadystatechange = function () {
                ...
                if (this.readyState === 4 && this.status === 200) {
                    ...
                    let imgs = doc.querySelectorAll("div.content-comic img");

                    pdfButton.imgs = [];
                    zipButton.imgs = [];
                    for(let img of imgs){
                        let src = img.dataset.src !== undefined ? img.dataset.src : img.src;
